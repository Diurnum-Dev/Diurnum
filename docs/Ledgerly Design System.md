...
> [!info] Purpose
> This is the canonical reference for Ledgerly's visual language and component patterns. Agents implementing UI features reference this document; the matching `tokens.css` file in the codebase carries the actual values. Both must stay in sync.
