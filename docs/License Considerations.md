Probably **not compatible with a closed-source bundled app** if Diurnum directly ships or links Beancount as part of the application.
...
That does **not** mean Diurnum is impossible. It means we need to be careful about how Diurnum uses Beancount.

## Practical implications

If Diurnum is going to be **open source under GPLv2-compatible terms**, then using Beancount directly is much easier.

If Diurnum is going to be **commercial/proprietary**, then directly bundling Beancount inside the macOS app as a Python sidecar or importing it as a library is legally risky. GPLv2 generally requires that distributed derivative/combined works be licensed under the GPL, so shipping Beancount as part of a proprietary desktop app is likely not acceptable without legal review or a separate license from the copyright holder.

## Safer architecture options

### Option 1: User-installed Beancount

Diurnum does not ship Beancount. Instead, it detects a user-installed Beancount CLI/library.
...
This is likely safer because Diurnum is not distributing Beancount. Downsides: worse onboarding, version mismatch, support burden.

### Option 2: Use Beancount format but implement your own parser/engine

Diurnum supports the Beancount **file format**, but does not include Beancount code.
...
### Option 3: Make Diurnum GPL/open source
...
If Diurnum wants to bundle Beancount in a proprietary product, ask the maintainer/copyright holder whether dual licensing is possible. The repo lists Martin Blais as author/copyright holder. ([GitHub](https://github.com/beancount/beancount?utm_source=chatgpt.com "GitHub - beancount/beancount: Beancount: Double-Entry Accounting from Text Files."))

## My recommendation

For Diurnum, I’d avoid making Beancount the embedded runtime dependency unless you are comfortable open-sourcing the desktop app under GPLv2-compatible terms.
...
2. **Write Diurnum’s own parser/validator for a constrained Beancount subset.**
...
> “Diurnum uses Beancount-compatible plain-text files as its ledger format.”

not:

> “Diurnum embeds Beancount under the hood.”

I’d run this by an open-source licensing attorney before distributing a paid/proprietary build, but the headline answer is: **GPLv2-only is a serious constraint for a proprietary bundled Diurnum app.**
