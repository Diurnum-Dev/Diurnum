...
- The Data Integrity layer writes `.bean` file changes with a write-temp, fsync, rename sequence. Current Source Account writes, Approval writes, transfer Approval writes, the account-rename refactor, and the Ledger Editor save command go through this boundary. Account rename snapshots all ledger files, backs up SQLite, and restores both on a failed multi-file mutation. Ledger Editor saves include the last-read file modification time so external edits produce a reload prompt instead of a silent overwrite.
...
- Settings owns the editable workspace metadata, source-account rename/close/opening-balance flows, source-mapping edits, AI adapter configuration, git identity, snapshot restore actions, and privacy toggles while still reusing the same underlying workspace data. The general Rename Ledger Account preview/apply commands update ledger references, Diurnum entry metadata, source-account SQLite rows, and source-account document folders, with explicit merge handling for an existing destination directive.
