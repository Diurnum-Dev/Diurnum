...
**V1**:
The first public Ledgerly release: the inherited MVP accounting loop assembled into an App Shell with a Ledger Editor home screen, Smart CSV Import, Documents, Settings, optional Git Integration, Data Integrity guarantees, and macOS packaging.
...
CSV statement direction columns where debit means money out of the statement account and credit means money into the statement account for Ledgerly CSV Imports.
...
**Manual Ledger Edit**:
A direct Founder-Operator edit to the Workspace's Beancount files outside Ledgerly's Approval flow, either in the Ledger Editor or in an external text editor.
_Avoid_: Unsupported edit, hidden database edit, Approval

**External Ledger Edit**:
A Manual Ledger Edit made outside Ledgerly while the app may be open.
...
The requirement that Ledgerly-written Workspaces remain valid Beancount V3 projects that can be inspected and validated with standard ecosystem tools.
_Avoid_: Ledgerly-only syntax, bundled Beancount fork, proprietary accounting database
...
**Local Desktop App**:
The product form for Ledgerly: a locally running desktop application that manages Workspace folders. V1 ships for macOS Apple Silicon, while the architecture should preserve a path to Windows and Linux later.
...
**Local-First V1**:
A V1 release where accounting data, Staging Area data, mappings, rules, documents, snapshots, reports, and validation all work without a Ledgerly cloud account.
...
The V1 guarantee that ledger file mutations use atomic writes, backup snapshots, restore safety, and crash recovery prompts before Ledgerly trusts derived data.
...
**Git Identity**:
The git `user.name` and `user.email` Ledgerly uses for local auto-commits and manual custom commits in a git-backed Workspace.
_Avoid_: Ledgerly cloud identity, Workspace owner, tax identity
...
**Privacy Boundary**:
The V1 rule that Ledgerly does not transmit accounting data, with outbound network behavior limited to configurable update checks, opt-in crash reports without Workspace data, and user-controlled BYO AI Adapter calls.
...
- Ledgerly is designed around an **MVP Business** for both the MVP and V1.
...
- **Beancount V3 Compatibility** is Ledgerly's exit promise: readable `.bean` files remain the canonical accounting source of truth.
- Ledgerly writes only valid Beancount V3 directives and must not introduce Ledgerly-only ledger syntax.
- Ledgerly may decline to import arbitrary external Beancount projects while still tolerating valid included `.bean` files without corrupting the Workspace.
- Ledgerly imports **Statement Rows** from bank-downloaded CSV files.
...
- AI may propose accounts, completions, explanations, or future query translations, but Ledgerly computes numbers from Workspace data and cites source entries when surfacing them.
...
- Ledgerly is a **Local Desktop App**. V1 packaging targets macOS Apple Silicon, but the architecture should not foreclose Windows and Linux later.
...
- The **Data Integrity Layer** applies to every `.bean` file mutation Ledgerly performs.
- Ledgerly creates a **Snapshot** before Approval and on a daily schedule; restoring a Snapshot creates a pre-restore Snapshot and runs Ledger Validation.
...
>
> **Dev:** "Does V1 still require users to edit `.bean` files outside Ledgerly?"
...
> **Domain expert:** "No. External Ledger Edits are detected and require an explicit reload decision before Ledgerly writes over them."
>
...
> **Domain expert:** "No. Ledger-Grounded AI means Ledgerly computes from Workspace data and cites the entries behind numbers."
>
...
>
> **Dev:** "Should Ledgerly create a git repository for every Workspace?"
...
- The app target could drift toward hosted web or Mac-only native; resolved: Ledgerly is a **Local Desktop App**. V1 ships on macOS Apple Silicon, while the architecture preserves a cross-platform path.
- "Local-first" could still hide cloud dependencies; resolved: the **Local-First MVP** and **Local-First V1** require no Ledgerly cloud account for accounting data.
...
- Beancount compatibility could mean Ledgerly must ingest every existing Beancount project; resolved: V1's compatibility is primarily an exit/interoperability promise, while arbitrary import is post-V1.
